...
class DividendCalculator:
    ...
    def calculate_dividend_yield(self, stock_code: str) -> Optional[DividendYield]:
        """
        计算单只股票的股息率
        ...
        """
        try:
            self.logger.info(f"计算股息率: {stock_code}")
            ...
            self.logger.info(f"股息率计算完成: {stock_code}, 股息率: {dividend_yield:.2f}%")
            return dividend_yield_obj
            
        except Exception as e:
            self.logger.error(f"计算股息率失败: {stock_code}, 错误: {str(e)}")
            return None
    ...
    def calculate_dividend_trend(self, stock_code: str, years: int = 5) -> Optional[DividendTrend]:
        """
        计算股息率趋势
        ...
        """
        try:
            self.logger.info(f"计算股息率趋势: {stock_code}, 年数: {years}")
            ...
            self.logger.info(f"股息率趋势计算完成: {stock_code}")
            return trend
            
        except Exception as e:
            self.logger.error(f"计算股息率趋势失败: {stock_code}, 错误: {str(e)}")
            return None
    ...
    def calculate_custom_yield(self, stock_code: str, dividend_amount: float) -> Optional[DividendYield]:
        """
        自定义股息率计算
        ...
        """
        try:
            self.logger.info(f"自定义股息率计算: {stock_code}, 分红金额: {dividend_amount}")
            ...
            self.logger.info(f"自定义股息率计算完成: {stock_code}, 股息率: {dividend_yield:.2f}%")
            return dividend_yield_obj
            
        except Exception as e:
            self.logger.error(f"自定义股息率计算失败: {stock_code}, 错误: {str(e)}")
            return None
    
    def get_batch_dividend_yields(self, stock_codes: List[str]) -> BatchDividendResult:
        """
        批量获取股息率
        ...
        """
        self.logger.info(f"批量获取股息率: {len(stock_codes)}只股票")
        
        dividend_yields = []
        
        for stock_code in stock_codes:
            try:
                dividend_yield = self.calculate_dividend_yield(stock_code)
                if dividend_yield:
        ...
        self.logger.info(f"批量获取完成: {len(dividend_yields)}/{len(stock_codes)}, 高股息股票: {len(high_dividend_stocks)}")
        return result
    
    def get_high_dividend_stocks(self, stock_codes: Optional[List[str]] = None, threshold: Optional[float] = None) -> List[DividendYield]:
        """
        获取高股息率股票
        
        :param stock_codes: 股票代码列表，如果为None则获取所有股票
        ...
        try:
            ...
            if stock_codes is None:
                stock_codes = self.fetcher.get_all_stock_list()
            
            self.logger.info(f"获取高股息率股票: 阈值={threshold}%, 股票数={len(stock_codes)}")
            ...
            for stock_code in stock_codes:
                try:
                    ...
                    self.logger.error(f"处理股票失败: {stock_code}, 错误: {str(e)}")
                    continue
    ...
    def analyze_dividend(self, stock_code: str) -> Optional[DividendAnalysisResult]:
        """
        综合分析股息率
        ...
        """
        try:
            self.logger.info(f"综合分析股息率: {stock_code}")
            
            dividend_yield = self.calculate_dividend_yield(stock_code)
            if not dividend_yield:
                return None
            
            dividend_trend = self.calculate_dividend_trend(stock_code)
            ...
            self.logger.info(f"综合分析完成: {stock_code}")
            return result
            
        except Exception as e:
            self.logger.error(f"综合分析失败: {stock_code}, 错误: {str(e)}")
            return None
